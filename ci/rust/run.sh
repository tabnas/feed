...
for SIBLING in parser json jsonic xml support debug; do
...
# the JSON core, the jsonic base grammar, the XML grammar, the fixture
# runner and the debug plugin). Those entries legitimately move whenever the
# sibling checkouts do, and exempting exactly them is what makes a full
