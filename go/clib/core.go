...
//
// libtabnasfeed is the feed format parser as a C shared library, one of
...
// newParser builds one engine with the feed grammar installed
...
func newParser() (parseFn, error) {
	j := host.Make(); if err := j.UseDefaults(plug.Feed, plug.Defaults); err != nil { return nil, err }; return j.Parse, nil
}
...
// parseWith answers whether src is in the feed language — and,
