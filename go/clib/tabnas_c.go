...
// Package main builds the C-ABI shared library: libtabnasfeed.
//
// tabnas-clib-template: v1 (stamped by admin tasks/adopt-clib.sh;
// edit the template and re-stamp, not this file).
//
//	go build -buildmode=c-shared -o libtabnasfeed.so ./clib
//
// feed TEXT IN, VERDICT (AND VALUE) OUT. This is one of the
...
// tabnas_grammar builds a feed parser and returns a handle to it.
...
// tabnas_parse checks one input against the feed grammar.
//
//export tabnas_parse
